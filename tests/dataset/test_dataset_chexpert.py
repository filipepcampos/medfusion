...
labels.loc[labels["Sex"] == "Unknown", "Sex"] = (
    "Female"  # Must be "female"  to match paper data
)
labels.fillna(3, inplace=True)
str_2_int = {
    "Sex": {"Male": 0, "Female": 1},
    "Frontal/Lateral": {"Frontal": 0, "Lateral": 1},
    "AP/PA": {"AP": 0, "PA": 1, "LL": 2, "RL": 3},
}
...
save_image(
    x,
    path_out / "samples_down_0.png",
    nrwos=int(math.sqrt(b)),
    normalize=True,
    scale_each=True,
)
...
for i in range(3):
    ...
    save_image(
        x_i,
        path_out / f"samples_down_{i+1}.png",
        nrwos=int(math.sqrt(b)),
        normalize=True,
        scale_each=True,
    )
