...
st.title("Chest X-ray images", anchor=None)
st.sidebar.markdown("Medfusion for chest X-ray image generation")
st.header("Information")
st.markdown(
    "Medfusion was trained on the [CheXpert](https://stanfordmlgroup.github.io/competitions/chexpert/) dataset",
)
...
cond_str = st.radio(
    ...
    ("Yes", "No"),
    index=1,
    help="Conditioned on 'cardiomegaly' or 'no cardiomegaly'",
    horizontal=True,
)
...
def init_pipeline():
    pipeline = DiffusionPipeline.load_from_checkpoint(
        "runs/2022_11_27_085654_chest_diffusion/last.ckpt",
    )
    return pipeline
