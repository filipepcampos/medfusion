...
st.markdown(
    "Medfusion was trained on the [CRC-DX](https://zenodo.org/record/3832231#.Y29uInbMKbg) dataset",
)
...
cond_str = st.radio(
    ...
    ("Yes", "No"),
    index=1,
    help="Conditioned on 'microsatellite stable (MSS)' or 'microsatellite instable (MSI)'",
    horizontal=True,
)
...
def init_pipeline():
    pipeline = DiffusionPipeline.load_from_checkpoint(
        "runs/2022_12_02_174623_patho_diffusion/last.ckpt",
    )
    return pipeline
