...
class VectorQuantizer(nn.Module):
    def __init__(self, num_embeddings, emb_channels, beta=0.25):
        ...
        self.embedder.weight.data.uniform_(
...
class VQVAE(BasicModel):
    def __init__(
        ...
        self.perceiver = (
            perceiver(**perceiver_kwargs).eval() if perceiver is not None else None
        )
    ...
    def _step(
        ...
        if self.global_step != 0 and self.global_step % self.sample_every_n_steps == 0:
            ...
            save_image(
                images,
                path_out / f"sample_{log_step}.png",
                nrow=x.shape[0],
                normalize=True,
            )
...
class VQGAN(VeryBasicModel):
    ...
    def vae_img_loss(self, pred, target, dec_out_layer, step, discriminator, depth=0):
        ...
        if step > self.start_gan_train_step:
            ...
            with torch.no_grad():
                self.log(
                    f"train/gan_loss_{depth}",
                    gan_loss,
                    on_step=True,
                    on_epoch=True,
                )
                self.log(
                    f"train/lambda_{depth}",
                    lambda_weight,
                    on_step=True,
                    on_epoch=True,
                )
            ...
            gan_loss = (
                0  # torch.tensor([0.0], requires_grad=True, device=target.device)
            )
    ...
    def _step(
        ...
        if optimizer_idx == 0:
            ...
            with torch.no_grad():
                logging_dict["img_loss"] = img_loss
            ...
            # Horizontal/Top Layer
            loss = (
                self.gan_img_loss(pred, target, step, self.discriminator, 0)
                * weights[0]
            )
            ...
            for i, pred_i in enumerate(pred_vertical):
                ...
                loss += (
                    self.gan_img_loss(pred_i, target_i, step, self.discriminator, i + 1)
                    * weights[i + 1]
                )
        ...
        # ----------------- Save Image ------------------------------
        if (
            self.global_step != 0 and self.global_step % self.sample_every_n_steps == 0
    ...
    def configure_optimizers(self):
        opt_vqvae = self.optimizer_vqvae(
            ...
            **self.optimizer_vqvae_kwargs,
        )
        opt_gan = self.optimizer_gan(
            self.discriminator.parameters(),
            **self.optimizer_gan_kwargs,
        )
        schedulers = []
        if self.lr_scheduler_vqvae is not None:
            schedulers.append(
                {
                    "scheduler": self.lr_scheduler_vqvae(
                        opt_vqvae,
                        **self.lr_scheduler_vqvae_kwargs,
                    ),
                    "interval": "step",
                    "frequency": 1,
                },
        ...
        if self.lr_scheduler_gan is not None:
            schedulers.append(
                {
                    "scheduler": self.lr_scheduler_gan(
                        opt_gan,
                        **self.lr_scheduler_gan_kwargs,
                    ),
                    "interval": "step",
                    "frequency": 1,
                },
    ...
    def _step(
        ...
        if self.global_step != 0 and self.global_step % self.sample_every_n_steps == 0:
            ...
            self.logger.log_image(
                key="samples",
                images=[depth2batch(img)[:16] for img in (x, pred)],
            )
