...
def compute_attention(q, k, v, num_heads, scale):
    q, k, v = map(
        lambda t: rearrange(t, "b (h d) n -> (b h) d n", h=num_heads),
        (q, k, v),
    )  # [(BxHeads), Dim_per_head, N]

    attn = (torch.einsum("b d i, b d j -> b i j", q * scale, k * scale)).softmax(
        dim=-1,
    )  # Matrix product = [(BxHeads), Dim_per_head, N] * [(BxHeads), Dim_per_head, N'] =[(BxHeads), N, N']

    out = torch.einsum(
        "b i j, b d j-> b d i",
        attn,
        v,
    )  # Matrix product: [(BxHeads), N, N'] * [(BxHeads), Dim_per_head, N'] = [(BxHeads), Dim_per_head, N]
    out = rearrange(
        out,
        "(b h) d n-> b (h d) n",
        h=num_heads,
    )  # ->  [B, (Heads x Dim_per_head), N]
...
class LinearTransformerNd(nn.Module):
    ...
    def __init__(
        ...
        ),  # Or use LayerNorm but be aware of https://github.com/pytorch/pytorch/issues/71465 (=> GroupNorm with num_groups=1)
        dropout=None,
        emb_dim=None,
    ):
        super().__init__()
        ...
        self.scale = (
            ch_per_head**-0.25
        )  # Should be 1/sqrt("queries and keys of dimension"), Note: additional sqrt needed as it follows OpenAI:  (q * scale) * (k * scale) instead of (q *k) * scale

        self.norm_x = get_norm_layer(
            norm_name,
        ...
        self.to_out = nn.Sequential(
            ...
            (
                nn.Identity()
                if dropout is None
                else get_dropout_layer(name=dropout, dropout_dim=spatial_dims)
            ),
        )
...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.self_atn = LinearTransformer(
            spatial_dims,
            in_channels,
            in_channels,
            num_heads,
        ...
        if emb_dim is not None:
            self.cros_atn = LinearTransformer(
                spatial_dims,
                in_channels,
                in_channels,
                num_heads,
