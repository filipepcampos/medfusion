...
class AIROGSDataset(SimpleDataset2D):
    def __init__(self, *args, **kwargs):
        ...
        self.labels = pd.read_csv(
            self.path_root.parent / "train_labels.csv",
...
class CheXpert_Dataset(SimpleDataset2D):
    ...
    def __getitem__(self, index):
        ...
        target = torch.tensor(
            self.labels.loc[uid, "Cardiomegaly"] + 1,
            dtype=torch.long,
...
class MIMIC_CXR_Dataset(SimpleDataset2D):
    def __init__(self, split_path, split="train", *args, **kwargs):
        ...
        labels = metadata_labels.merge(
            ...
            on="study_id",
            how="left",
        )  # TODO: Rethink whether outer is necessary
        ...
        def get_path(row):
            ...
            subject = "p" + str(
                int(row["subject_id"]),
            )  # todo: this is a lazy hack instead of properly changing column type
...
class Brax(SimpleDataset2D):
    attr_names = ["Cardiomegaly"]
    prefix = "files/brax/1.1.0/"

    def __init__(self, split="train", data_filter=None, *args, **kwargs):
        ...
        self.data = (
            val_df if split == "valid" else train_df if split == "train" else test_df
        )
    ...
    def __getitem__(self, idx):
        ...
        img = Image.open(os.path.join(self.path_root, self.prefix, img_path)).convert("RGB")
        ...
        return {"source": self.transform(img), "target": attr}
