    ...
    def kl(self, other=None):
        if self.deterministic:
            ...
            if other is None:
                return (
                    0.5
                    * torch.sum(torch.pow(self.mean, 2) + self.var - 1.0 - self.logvar)
                    / self.batch_size
                )
            else:
                return (
    ...
    def _step(
        ...
        if optimizer_idx == 0:
            ...
            loss = (
                ...
                + rec_loss
                + self.embedding_loss_weight * vq_emb_loss
            )
        ...
        with torch.no_grad():
            results["L2"] = torch.nn.functional.mse_loss(pred, x)
            results["L1"] = torch.nn.functional.l1_loss(pred, x)
...
class VAEWrapper(BasicModel):
    def __init__(
        ...
        self.logvar = nn.Parameter(
            torch.zeros(size=()),
        )  # Better weighting between KL and MSE, see (https://arxiv.org/abs/1903.05789), also used by Taming-Transfomer/Stable Diffusion
    ...
    def encode(self, x):
        z = self.model.encode(
            x,
        )  # Latent space but not yet mapped to discrete embedding vectors
    ...
    def _step(
        ...
        # ------------------------- Compute Loss ---------------------------
        loss = (
            torch.sum(torch.square(pred - target)) / x.shape[0]
        )  # torch.sum( torch.square((pred-target)/torch.exp(self.logvar))/2 + self.logvar + HALF_LOG_TWO_PI )/x.shape[0]
        loss += kl_loss
