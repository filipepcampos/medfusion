...
class ImprovedPrecessionRecall(Metric):
    ...
    def update(self, imgs: Tensor, real: bool) -> None:  # type: ignore
        ...
        """
        assert (
            torch.is_tensor(imgs) and imgs.dtype == torch.uint8
...
def _compute_metric(ref_features, ref_radii, ref_splits, pred_features, pred_splits):
    dist = _compute_pairwise_distances(
        ref_features,
        ref_splits,
        pred_features,
        pred_splits,
        ...
        [
            torch.concat(
                [
                    ...
                    for Y_batch in Y.chunk(splits_y, dim=0)
                ],
                dim=1,
            )
